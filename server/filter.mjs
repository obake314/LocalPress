...
const ALWAYS_EXCLUDE = [
  ["記者会見", /記者会見|定例会見|会見録/],
  // 入札は入札情報公開サービスから取るため、役所サイトのRSS経由では拾わない。
  // 同じ案件が二重に入るうえ、あちらは入札方式・工種・開札日まで揃っている。
  ["入札（別経路で取得）", /入札公告|入札結果|落札結果|指名停止|条件付一般競争入札|公募型プロポーザル|指名競争入札|開札/],
...
const EXCLUDE_UNLESS_ANNOUNCEMENT = [
  ...
  // 資格・免許試験の実施案内。職員採用試験は雇用の募集なので下の除外条件で残す
  ["資格試験", /師試験|士試験|検定試験|資格試験|免許試験|技能検定/],
  // 工事の進捗報告や通行規制。入札公告や発注予定公表は案件なので残す
  ["工事進捗・交通規制", /工事進捗|進捗状況|瓦版|工事だより|通行規制|通行止|全面通行|工事が.{0,14}始まります/],
