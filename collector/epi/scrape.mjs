...
async function scrapeOrg(page, org) {
  ...
  const result = frame(page, "KFK301FrameShow")
  if (!result) return []
  ...
  return result.evaluate(() =>
    [...document.querySelectorAll("table tr")]
      ...
      .filter((r) => r.cells.length >= 7 && /^\d{4}\//.test(r.cells[0]))
  )
}
...
for (const org of ORGS.orgs) {
  ...
  try {
    const rows = await scrapeOrg(page, org)
    ...
    console.log(`  ${org.name.padEnd(8, "　")} ${rows.length} 件`)
  } catch (e) {
    console.log(`  ${org.name.padEnd(8, "　")} 取得失敗: ${e.message}`)
  }
